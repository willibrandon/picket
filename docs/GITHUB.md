...
Picket fetches each selected gist detail before scanning files. It warns when GitHub reports a truncated file list, scans inline gist file content when available, and scans gist comments through the gist comments API.

### Failure Handling

Repository tree truncation and per-file download failures are warnings. Organization and user scans also treat per-repository tree failures as warnings and continue with the remaining repositories.
