...
GCS scans use OAuth bearer-token REST requests for the Cloud Storage JSON API `objects.list` and `objects.get` methods.

```powershell
picket scan --gcs-bucket secrets --gcs-token-env PICKET_GCS_TOKEN --report-format jsonl
```

Object names can be filtered by prefix:

```powershell
picket scan --gcs-bucket secrets --gcs-prefix prod/ --gcs-token-env PICKET_GCS_TOKEN --report-format jsonl
```

Requester Pays buckets can include a billing project:

```powershell
picket scan --gcs-bucket secrets --gcs-prefix prod/ --gcs-token-env PICKET_GCS_TOKEN --gcs-user-project billing-project --report-format jsonl
```

The bearer token is read from an environment variable and is never passed as a command-line value.

| Option | Purpose |
| --- | --- |
| `--gcs-bucket` | Bucket to enumerate. |
| `--gcs-endpoint` | Optional Cloud Storage JSON API endpoint. Defaults to `https://storage.googleapis.com/`. |
| `--gcs-prefix` | Optional object name prefix. |
