...
Native fingerprints are versioned as `picket:v1:<sha256>`. The hash input includes the normalized logical path, rule ID, secret or match hash, and decode path. It intentionally excludes line, column, commit, author, and message metadata so native triage IDs remain stable when a finding moves inside the same file or appears across multiple commits. Container archive findings use the logical in-image path rather than the archive filename or compressed layer blob digest, while the report path retains the complete archive and layer provenance. Gitleaks-compatible reports keep Gitleaks fingerprints. The OpenAI API-key family rules introduced after the original grouped rule use `picket-openai-api-key` as their v1 fingerprint identity, so existing report fingerprints and `.picketignore` entries remain valid.

Native Git changes reports distinguish `git-index`, `git-worktree`, and `git-untracked` snapshots. When the same stable finding occurs in both index and working-tree content, Picket pairs occurrences and emits `git-index+worktree` with the current working-tree coordinates. Findings unique to either snapshot remain separate.

Baseline comparison is exact by default. Native commands can select `--baseline-mode portable` to treat LF, CRLF, and CR evidence as equivalent while requiring all other baseline fields and source coordinates to match. Strict compatibility always uses exact comparison. A native baseline containing the former grouped `picket-openai-api-key` identity suppresses an otherwise matching legacy, project, project service-account, or organization-admin OpenAI API-key finding. Baselines written with the new family-specific identities remain distinct.
...
`picket verify secret` writes one compact `picket.validation.v1` JSON object to standard output. The object contains `state`, `provider`, `ruleId`, `reason`, `identity`, `scopes`, `reachableResources`, and `evidence`. These fields are restricted to non-secret provider metadata. The direct-verification output never includes the credential, match text, source line, or secret hash.

Revocation command templates never include raw secret values. Picket emits concrete provider commands when the finding includes enough non-secret identifiers, such as an AWS access key ID, Azure Storage account name, or GCP service-account key ID. GitHub token reports emit `picket revoke github --credential-env PICKET_GITHUB_CREDENTIAL --confirm-revocation`; the operator must place the credential in that environment variable before explicitly running the guarded command. GitLab personal access token reports emit `picket revoke gitlab --credential-env PICKET_GITLAB_CREDENTIAL --confirm-revocation`, which uses GitLab's self-revocation endpoint. Other GitLab token families include placeholder-based API or runner commands only when they have a safe owning-record workflow, such as deploy tokens, pipeline triggers, and runner authentication tokens. When the provider supports revocation but the report cannot safely or accurately produce an exact command, `revocationAvailable` remains `true`, `revocationCommands` is empty or placeholder-based, and `revocationGuidance` explains the provider workflow. `picket revoke` uses typed provider clients and does not execute report command strings.
...
For Gitleaks-compatible commands, `--verbose` writes human-readable finding records to standard output. Git-history findings are written as fragment scans complete, and directory findings are written as files complete, rather than after the complete scan has been buffered. Ignore, baseline, and redaction filters run before a finding is written. Colored output includes up to 20 characters of source-line context on each side of the match, following Gitleaks. The banner and scan summary are written to standard error. These console records are independent of `--report-path`; selecting a report does not suppress them. `--no-banner`, `--no-color`, and `--log-level` control the compatibility console without changing report bytes or their deterministic complete-scan ordering.
