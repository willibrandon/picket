...
Dashboard, finding details, and logs are read-only text panes so terminal users can select and yank exact text with the mouse or keyboard. Rule and file rows can filter the Findings view to the selected rule or file. Findings and file rows can open local files when the path exists on disk; Picket uses `PICKET_EDITOR`, then `VISUAL`, then `EDITOR`, with line-aware arguments for GUI editors, and falls back to the operating system default file opener. Terminal editors such as `nvim` are rejected inside the full-screen TUI because they need exclusive terminal control.
