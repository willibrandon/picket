...
Azure DevOps source support is native Picket behavior, not Gitleaks compatibility behavior. It currently supports both cloud and self-hosted endpoints, project-scoped and organization-scoped Azure Repos enumeration, PAT and job-token authentication where each is appropriate, continuation tokens capped at a 1,000-page safety limit per paged list, retryable throttling responses retried once with bounded `Retry-After` backoff, responses from injected HTTP handlers that already followed a redirect rejected, branch scope controls, pull-request source-head scans through `--azure-devops-pull-request`, fork source repositories when Azure DevOps returns them, optional wiki backing repository scans through `--azure-devops-include-wikis`, build-scoped artifact scanning through `--azure-devops-include-artifacts`, build-scoped log scanning through `--azure-devops-include-logs`, classic release build artifact scanning through `--azure-devops-include-release-artifacts`, a 100 decimal MB default remote download cap, positive file-size caps, positive artifact/log caps, archive safety caps, HTTPS credential transport by default with only explicit insecure opt-in for public HTTP endpoints, credential-free signed redirect downloads, and clear handling for repositories, projects, wikis, artifacts, release artifacts, or logs the token cannot read. Feed/package sources and artifact/log redaction before diagnostics remain planned explicit opt-ins. Pipeline task defaults scan the job's checked-out workspace unless the user explicitly opts into remote Azure DevOps enumeration.
...
The full-screen console is an operator interface, not a marketing screen. Opening without a report starts on the Scan page; opening a report with findings starts on Findings. The Scan page is for setting up and running a scan: Run scan, target inputs, command preview, status, exit code, scan timing, report path, result count summary, and recent scanner output. The Findings page owns row triage: filtering, selected-row focus, finding details, and finding-specific yank text. This prevents the Scan page from becoming a duplicate findings browser while still making the next action obvious after a scan completes. It favors dense controls, stable row keys where tables are used, clear focus, keyboard navigation, and text labels over decorative graphics.

The scanner console also has a native scan workspace. The scan workspace lets users choose a local path or source-host target, profile, config, ignore behavior, verification mode, result filter, archive limits, redaction, and report outputs through terminal controls. It builds and displays the command-equivalent `picket scan` request, prepares the report output directory, invokes the scanner executable instead of reimplementing scan behavior in the TUI, displays text status, exit code state, started/completed/elapsed-time diagnostics, streamed stdout/stderr scanner output, and cancellation status, then reloads the generated report summary when the scan completes. Completed scans expose the loaded finding count and a direct `g f` path to the Findings page rather than duplicating the findings table on Scan. While a scan is running, the primary action changes from Run scan to Cancel and `Ctrl+C` requests scan cancellation without closing the console. It must not create a separate behavior path from the CLI.
