...
- One shared Aho-Corasick keyword prefilter maps matched keywords to candidate rules before independent per-rule regex execution. Rules without keywords remain unconditional.
...
- Local files larger than 100,000 bytes are streamed through pooled fragments instead of being materialized as one managed array. Strict compatibility reproduces Gitleaks' Go buffered reader: a primary read requests up to 100,000 bytes, read-ahead consumes at most 25,000 bytes through the 4 KiB buffer while seeking a blank-line boundary, and any buffered remainder becomes the next short primary read. Fragments do not overlap. Native filesystem and baseline scans inspect the same fragments plus the final 64 KiB of the preceding fragment, expanded backward to a line boundary when available, and deduplicate overlap findings by rule, absolute location, evidence, and decode path. Native scans also recheck `gitleaks:allow` with a bounded complete-source pass when a finding occurs on a force-cut line, so the marker remains effective even when one attacker-controlled line spans more than two fragments. That pass verifies the complete source hash instead of concatenating an unbounded line in memory. This preserves Gitleaks' hard-boundary behavior on the compatibility surface while allowing native rules and decoders to detect ordinary cross-boundary secrets.
- Native scans decode BOM-marked UTF-16 LE and BE before matching. Small inputs use cleared pooled UTF-8 storage; fragmented files and standard input use a stateful transcoding stream so code units and UTF-8 scalars remain intact across bounded reads. Malformed code units become `U+FFFD`. Size limits and complete-source SHA-256 identities remain anchored to the original encoded bytes. Strict Gitleaks-compatible commands do not enable this behavior and classify only the first 100,000 bytes with the pinned Gitleaks file-type rules, skipping a file only when the detected top-level MIME type is `application`.
- Fragment positions retain the original one-based line and column. Native fragment boundaries preserve complete UTF-8 scalars and advance columns by Unicode code point; compatibility fragments retain UTF-8 byte columns. Reports and native cache entries use the SHA-256 identity of the complete file rather than a fragment hash. The first 100,000 bytes provide the Gitleaks-compatible binary probe; safe-boundary read-ahead bytes do not affect classification. Native content-hash ignores and cache lookups prehash the bounded stream. When scanning continues after that prehash, Picket rejects a file that changes before its scan completes.
...
`-l/--log-level`, `-v/--verbose`, `--no-color`, and `--no-banner` are recursive root options and may appear before or after a compatibility command. Unless `--no-banner` is selected, compatibility scans write the Picket banner to standard error. `--verbose` writes each filtered finding to standard output using the Gitleaks field order, including supporting `Required` evidence for composite rules. Git findings are written as fragments complete and directory findings as files complete; reports retain deterministic complete-scan ordering. Scan byte totals, elapsed time, commit counts, and the final leak count are written to standard error and filtered by `--log-level`. Requested reports remain separate file or standard-output artifacts governed by the report options.
...
Strict compatibility enables Scout's `MatchInvalidUtf8` option so each malformed byte is evaluated as one `U+FFFD` replacement rune, matching Go string iteration without copying or expanding the input. Match offsets, size limits, blob hashes, report locations, and fingerprints remain anchored to the original bytes.
...
Native mode adds `.picketignore`, glob ignores, content-hash ignores, stale-ignore auditing, and stable fingerprints. A `.picketignore` entry can be a complete `picket:v1:<sha256>` fingerprint copied from a native report to suppress one stable finding, or `sha256:<content-sha256>` to suppress a complete file by content identity. After successful local native scans and baseline creation, stale `sha256:` entries that did not match any scanned file produce stderr warnings with the ignore-file location when known. When a built-in native rule is split into privilege-specific identities, its v1 fingerprint identity and baseline aliases are preserved explicitly for migration rather than changing existing suppressions. These features never silently affect strict Gitleaks compatibility.
...
- global and per-provider pacing and concurrency limits, including a default same-provider interval and native CLI overrides in milliseconds,
- absolute verifier-lifetime request budgets of 100 total outbound attempts and 25 attempts for any one provider by default, with positive native CLI overrides; retries consume capacity while cache hits and preflight rejections do not,
...
`picket revoke github` reads each credential only from a named environment variable supplied through repeatable `--credential-env` options and requires `--confirm-revocation`. It accepts only credential families documented by GitHub's revocation API, sends an unauthenticated `POST /credentials/revoke` request, and never puts credentials in command arguments, output, diagnostics, response evidence, or cache entries. The request is limited to 1,000 credentials, guarded before DNS connection and again at socket connect time, HTTPS-only, non-public-address-denying by default, redirect-free, and non-retrying. A `202` response means GitHub accepted the request; validation failures are rejected; transport failures, timeouts, unexpected success responses, and server errors are indeterminate because the mutation may already have reached the provider.

`picket revoke gitlab` reads one personal access token from a named environment variable, requires `--confirm-revocation`, and sends a bodyless `DELETE` to GitLab's `/api/v4/personal_access_tokens/self` endpoint with the token only in the `PRIVATE-TOKEN` header. Endpoint overrides support GitLab Self-Managed relative URL roots but must end in the documented self-revocation path. The request uses the same preflight and connect-time address policy as GitHub revocation, permits only HTTPS, does not follow redirects, and is never retried. A `204` response confirms revocation; client errors are rejected; transport failures, timeouts, unexpected success responses, and server errors are indeterminate.

Provider-specific revocation commands must preserve these irreversible-operation semantics rather than reusing live-verification retry or cache behavior. Scanner, verification, and analysis code cannot construct revocation clients; only the explicit revoke command owns that boundary.
...
The workspace displays the command-equivalent `picket scan` request and runs that scanner executable. It shows an animated activity indicator together with text status, exit code, start/completion/elapsed timing, output availability, and cancellation state. The Logs view owns captured stdout and stderr.
...
`F1` and `?` open an in-app keyboard reference generated from the active Hex1b input bindings. Number keys `1` through `6` and global `g` chords cover Dashboard, Scan, Findings, Rules, Files, and Logs; number keys retain their text-entry behavior while an editable text field has focus. `Tab` follows visual reading order through every active-page control and wraps, while `Shift+Tab` traverses the same focus ring in reverse. Both override editor capture so text fields and read-only panes cannot trap keyboard focus. Contextual footer hints remain limited to actions relevant to the active page. Plain and selection-based yank operations visibly flash the copied text or table row. Selectable read-only panes suppress editor filler markers without giving up mouse selection, keyboard selection, or yank behavior. A non-empty `NO_COLOR` environment variable selects a monochrome palette while preserving visible focus and textual semantic labels. Arbitrary third-party Hex1b themes are not exposed because the shipped palette and its monochrome variant are the tested accessibility contract.
...
The GitHub Action supports mutually exclusive workspace path, Docker archive, OCI archive, and registry image sources. Omitting every source selector scans the GitHub workspace. Archive paths resolve from the workspace, registry image references remain literal, and registry credentials are supplied only by environment variable name through either bearer-token or complete Basic authentication inputs. Source selection retains the Action's annotations, SARIF upload, fetch-depth guidance, baseline and native-ignore handling, validation-result filtering, cache restore/save, least-privilege permissions, summary output, explicit endpoint exceptions, and fail modes. Generated reports and cache data default beneath the runner temporary directory so scanning the checkout cannot ingest output from an earlier run; explicit paths remain supported.

The Azure DevOps pipeline integration supports a first-class `PicketScan@1` task, workspace scans, Docker archives, OCI archives, registry images, optional Azure Repos/project/org enumeration, pipeline/build/release artifact scanning, native ignore files, JSONL/SARIF/HTML report publication, build annotations where Azure DevOps supports safe source locations, explicit fail modes, scan-cache use through task inputs, and no telemetry. Workspace, container, registry, and Azure DevOps remote sources are mutually exclusive, and omitting all source inputs scans `$(Build.SourcesDirectory)`. Registry authentication and endpoint policy follow the same environment-name-only and explicit-opt-in contract as the GitHub Action. Task metadata lives under `azure-devops/tasks/PicketScanV1`, and the marketplace extension manifest lives at `azure-devops/vss-extension.json`. Release-phase packaging validates the VSIX and agent smoke tests before publication. Self-hosted marketplace smoke tests run only through explicit manual queueing; pull-request and push validation use hosted agents, recorded responses, or local fakes. The task works on Microsoft-hosted and self-hosted Windows, Linux, and macOS agents when `picket` is available through `picketPath` or the agent `PATH`.
...
Gate: release workflow produces signed/checksummed artifacts, action smoke tests pass, stable releases publish the Azure DevOps extension and verify the exact GitHub Marketplace Action release after owner publication, and the docs site builds and deploys through GitHub Pages from the default branch.
