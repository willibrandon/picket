...
Azure DevOps source support is native Picket behavior, not Gitleaks compatibility behavior. It currently supports both cloud and self-hosted endpoints, project-scoped and organization-scoped Azure Repos enumeration, PAT and job-token authentication where each is appropriate, continuation tokens, branch scope controls, pull-request source-head scans through `--azure-devops-pull-request`, fork source repositories when Azure DevOps returns them, optional wiki backing repository scans through `--azure-devops-include-wikis`, file-size caps, and clear handling for repositories, projects, or wikis the token cannot read. Build artifacts, release artifacts, pipeline logs, rate-limit backoff, and artifact/log redaction before diagnostics remain planned explicit opt-ins. Pipeline task defaults scan the job's checked-out workspace unless the user explicitly opts into remote Azure DevOps enumeration.
