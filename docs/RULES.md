...
Native commands currently use the same rule model with Picket-native environment precedence. Gitleaks-compatible `dir`/`file`/`directory` and `stdin` scans use this same native precedence only when `--profile picket` is supplied.
