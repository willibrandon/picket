...
Only one native source provider can be selected for a scan. Do not combine `--docker-archive` or `--oci-archive` with GitHub, Gitea, GitLab, Azure DevOps, or object-store source enumeration flags.
