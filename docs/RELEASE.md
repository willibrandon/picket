...
Both integrations pin the downloaded Dotsider CLI to `v0.24.9`. GitHub enforces absolute caps of 25 MB for Linux and Windows and 40 MB for macOS; Azure enforces 25 MB for its Windows x64 build. The separate macOS cap accounts for the release profile's intentional `StripSymbols=false` workaround while still making a material increase fail CI.
