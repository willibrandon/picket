...
- `ghcr.io/willibrandon/picket:<release-tag>`, for example `v0.1.11`,
- `ghcr.io/willibrandon/picket:<semver>`, for example `0.1.11`,
...
The release tag is the source of truth for package versions. Release jobs strip a leading `v` from tags such as `v0.1.11`, pass the resulting SemVer value to `Version` and `PackageVersion`, publish package and symbol files with `--skip-duplicate`, and fail clearly when `NUGET_API_KEY` is missing.
