...
Both integrations pin the downloaded Dotsider CLI to `v0.25.1`. GitHub enforces absolute caps of 25 MB for Linux and Windows and 40 MB for macOS; Azure enforces 25 MB for its Windows x64 build. The separate macOS cap accounts for the release profile's intentional `StripSymbols=false` workaround while still making a material increase fail CI.

Dotsider manages a separate verified baseline for each workflow job, logical target, and RID. Successful `main` builds retain the executable and `.mstat` sidecar; pull requests compare with the newest compatible successful `main` build without project-specific artifact queries. Each integration supplies a stable baseline key so temporary runner paths cannot split one RID's history across separate baselines. When no retained baseline exists, Dotsider enforces the absolute cap, names growth limits as deferred, and establishes the baseline after the next successful `main` build. Pull-request artifacts and the current build are never eligible as their own baseline. These baseline sidecars exist only as CI artifacts and are not added to release archives.
...
- `ghcr.io/willibrandon/picket:<release-tag>`, for example `v0.2.11`,
- `ghcr.io/willibrandon/picket:<semver>`, for example `0.2.11`,
...
```powershell
dotnet run --file ./scripts/Generate-PackageManagerManifests.cs -- -ReleaseTag v0.2.11 -ChecksumsPath dist/checksums.txt -OutputDirectory artifacts/package-managers -Clean
```
...
The release tag is the source of truth for package versions. Release jobs strip a leading `v` from tags such as `v0.2.11`, pass the resulting SemVer value to `Version` and `PackageVersion`, publish package and symbol files with `--skip-duplicate`, and fail clearly when `NUGET_API_KEY` is missing.
