...
- `ghcr.io/willibrandon/picket:<release-tag>`, for example `v0.2.3`,
- `ghcr.io/willibrandon/picket:<semver>`, for example `0.2.3`,
...
```powershell
dotnet run --file ./scripts/Generate-PackageManagerManifests.cs -- -ReleaseTag v0.2.3 -ChecksumsPath dist/checksums.txt -OutputDirectory artifacts/package-managers -Clean
```
...
The release tag is the source of truth for package versions. Release jobs strip a leading `v` from tags such as `v0.2.3`, pass the resulting SemVer value to `Version` and `PackageVersion`, publish package and symbol files with `--skip-duplicate`, and fail clearly when `NUGET_API_KEY` is missing.
