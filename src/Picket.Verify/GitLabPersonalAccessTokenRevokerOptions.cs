...
/// <summary>
/// Configures explicit GitLab personal access token self-revocation requests.
/// </summary>
public sealed class GitLabPersonalAccessTokenRevokerOptions
{
    private const string SelfRevocationPath = "/api/v4/personal_access_tokens/self";
    ...
    private Uri _credentialEndpoint = new("https://gitlab.com/api/v4/personal_access_tokens/self");
    private Func<HttpMessageHandler>? _messageHandlerFactory;
    private Uri? _proxyEndpoint;
    private TimeSpan _timeout = TimeSpan.FromSeconds(10);

    /// <summary>
    /// Creates default GitLab personal access token revoker options.
    /// </summary>
    /// <returns>Default GitLab personal access token revoker options.</returns>
    public static GitLabPersonalAccessTokenRevokerOptions CreateDefault()
    {
        return new GitLabPersonalAccessTokenRevokerOptions();
    }

    /// <summary>
    /// Gets or sets the GitLab personal access token self-revocation endpoint.
    /// </summary>
    public Uri CredentialEndpoint
    {
        ...
        set
        {
            ...
            {
                throw new ArgumentException("Endpoint URI must use HTTPS.", nameof(value));
            }
            ...
            if (!value.AbsolutePath.EndsWith(SelfRevocationPath, StringComparison.Ordinal))
            {
                throw new ArgumentException(
                    $"Endpoint URI path must end with {SelfRevocationPath}.",
                    nameof(value));
            }
    ...
    /// <summary>
    /// Gets or sets the HTTP user agent sent to GitLab.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets the optional HTTPS proxy endpoint used for GitLab API requests.
    /// </summary>
