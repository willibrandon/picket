...
internal static class PicketTuiApp
{
        ...
        where TParent : Hex1bWidget
    {
        return ctx.VStack(v => [
            ...
            BuildSectionGap(v),
            BuildSectionTitle(v, "Live request budget"),
            BuildBlankLine(v),
            BuildTextField(v, "Total", scan.LiveMaxRequests, scan.SetLiveMaxRequests),
            BuildTextField(v, "Per provider", scan.LiveMaxRequestsPerProvider, scan.SetLiveMaxRequestsPerProvider),
            BuildWideGap(v),
        ...
        where TParent : Hex1bWidget
    {
        return ctx.HStack(h => [
            ...
            h.VStack(right => [
                ...
                BuildBooleanField(right, "Issues", scan.IncludeGitLabIssues, scan.SetIncludeGitLabIssues),
                BuildChoiceField(right, "Issue state", PicketTuiScanWorkspace.GitLabIssueStates, scan.GitLabIssueStateIndex, scan.SetGitLabIssueStateByIndex),
                BuildBooleanField(right, "Releases", scan.IncludeGitLabReleases, scan.SetIncludeGitLabReleases),
                BuildBooleanField(right, "Release assets", scan.IncludeGitLabReleaseAssets, scan.SetIncludeGitLabReleaseAssets),
                BuildBooleanField(right, "Non-public", scan.AllowNonPublicSourceEndpoints, scan.SetAllowNonPublicSourceEndpoints),
                BuildBooleanField(right, "HTTP", scan.AllowInsecureSourceEndpoints, scan.SetAllowInsecureSourceEndpoints),
            ]).FillWidth(),
        ]).FillWidth();
