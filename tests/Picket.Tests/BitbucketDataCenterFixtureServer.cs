...
internal sealed class BitbucketDataCenterFixtureServer : IDisposable
{
    ...
    internal BitbucketDataCenterFixtureServer(string content)
    {
        _content = content;
        _listener = new TcpListener(IPAddress.Loopback, 0);
        _listener.Start();
        int port = ((IPEndPoint)_listener.LocalEndpoint).Port;
        Endpoint = new Uri($"http://127.0.0.1:{port}/rest/api/1.0/");
    ...
    private async Task HandleAsync(TcpClient client, CancellationToken cancellationToken)
    {
        ...
        if (target.Equals("/rest/api/1.0/projects/CORE/repos/picket/default-branch", StringComparison.Ordinal))
        {
            string json = $$"""{"id":"refs/heads/main","latestCommit":"{{Commit}}"}""";
            await WriteResponseAsync(stream, "application/json", Encoding.UTF8.GetBytes(json), cancellationToken).ConfigureAwait(false);
            return;
        }

        if (target.Equals($"/rest/api/1.0/projects/CORE/repos/picket/files?at={Commit}&limit=100&start=0", StringComparison.Ordinal))
        {
            const string FilesJson = """{"isLastPage":true,"values":["src/appsettings.txt"]}""";
            await WriteResponseAsync(stream, "application/json", Encoding.UTF8.GetBytes(FilesJson), cancellationToken).ConfigureAwait(false);
            return;
        }

        if (target.Equals($"/rest/api/1.0/projects/CORE/repos/picket/raw/src/appsettings.txt?at={Commit}", StringComparison.Ordinal))
        {
