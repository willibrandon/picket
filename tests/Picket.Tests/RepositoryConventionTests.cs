...
public sealed partial class RepositoryConventionTests
{
    ...
    private static readonly string[] s_projectDocsSidebarItems =
    [
    ...
    {
        string githubWorkflow = ReadRepositoryFile(".github/workflows/ci.yml");
        string azurePipeline = ReadRepositoryFile("azure-pipelines.yml");
        string documentation = ReadRepositoryFile("docs/RELEASE.md");
        ...
        Assert.Contains("baseline-key: picket-${{ matrix.rid }}", githubWorkflow);
        Assert.DoesNotContain("Find latest Native AOT size baseline", githubWorkflow);
        Assert.DoesNotContain("actions/download-artifact", githubWorkflow);
        Assert.DoesNotContain("baseline: ${{ steps.size_baseline.outputs.path }}", githubWorkflow);
        ...
        Assert.Contains("newest compatible successful `main` build", documentation);
        Assert.Contains("without project-specific artifact queries", documentation);
        Assert.Contains("Pull-request artifacts and the current build are never eligible as their own baseline", documentation);
    ...
    public void AzureDevOpsTaskMetadataDocumentsScannerContract()
    {
        ...
        Assert.AreEqual("0.2.11", extensionRoot.GetProperty("version").GetString());
        ...
        Assert.Contains("## 0.2.11", changelog);
    ...
    public void ReleaseWorkflowAutomatesSupportedMarketplaceStepsAndFailsClosed()
    {
        ...
        Assert.DoesNotContain("--share-with", workflow);
    ...
    public void ReleaseWorkflowCreatesChecksummedAttestedArtifacts()
    {
        ...
        Assert.Contains("$startInfo = [Diagnostics.ProcessStartInfo]::new()", workflow);
        Assert.Contains("[void]$startInfo.ArgumentList.Add('tui')", workflow);
        Assert.Contains("$companionExitCode = $companionProcess.ExitCode", workflow);
        Assert.DoesNotContain("& $picket tui $missingReport", workflow);
    ...
    public void DirectoryBuildPropsDefinesPackageMetadata()
    {
        ...
        AssertProjectProperty(props, "VersionPrefix", "0.2.11");
    ...
    public void RequiredDocumentationDeliverablesCoverCurrentContracts()
    {
        ...
        string tui = ReadRepositoryFile("docs/TUI.md");
        string action = ReadRepositoryFile("docs/ACTION.md");
        string github = ReadRepositoryFile("docs/GITHUB.md");
        string gitlab = ReadRepositoryFile("docs/GITLAB.md");
        ...
        string docsSitePackage = ReadRepositoryFile("docs-site/package.json");
        string docsGeneratorProject = ReadRepositoryFile("tools/Picket.Docs/Picket.Docs.csproj");
        string docsWorkflow = ReadRepositoryFile(".github/workflows/docs.yml");
        ...
        Assert.Contains("Reference Clone Discovery", upstream);
        Assert.Contains("Capture-UpstreamPins.cs", upstream);
        Assert.Contains("Microsoft Learn", upstream);
        Assert.Contains("System.CommandLine", upstream);
        Assert.Contains("GitHub Source References", upstream);
        Assert.Contains("GitLab Source References", upstream);
        Assert.Contains("Azure DevOps Source References", upstream);
        Assert.Contains("reviewed on", upstream);
        ...
        Assert.Contains("picket tui --scan", tui);
        Assert.Contains("picket-tui --scan", tui);
        Assert.Contains("--flow", tui);
        Assert.Contains("PICKET_EDITOR", tui);
        Assert.Contains("Hex1b tests", tui);
        Assert.Contains("WCAG 2.2 AA", tui);
        Assert.Contains("picket.sarif", action);
        Assert.Contains("picket.jsonl", action);
        Assert.Contains("fail-on", action);
        Assert.Contains("annotations", action);
        Assert.Contains("redact", action);
        Assert.Contains("actions/cache/restore", action);
        Assert.Contains("Actions artifacts", github);
        Assert.Contains("Link` pagination header", github);
        Assert.Contains("without forwarding the bearer token", github);
        Assert.Contains("Hosted Alert Oracle", github);
        Assert.Contains("PICKET_GITHUB_SECRET_SCANNING_PAT", github);
        Assert.Contains("Contents Read", github);
        Assert.Contains("--gitlab-include-job-artifacts", gitlab);
        Assert.Contains("--gitlab-include-job-logs", gitlab);
        Assert.Contains("--gitlab-include-packages", gitlab);
        Assert.Contains("PRIVATE-TOKEN", gitlab);
        Assert.Contains("generic package", gitlab);
        Assert.Contains("1,000 pages", gitlab);
        ...
        Assert.Contains("S3 buckets", objectStores);
        Assert.Contains("Google Cloud Storage buckets", objectStores);
        Assert.Contains("Azure Blob Storage containers", objectStores);
        Assert.Contains("SigV4", objectStores);
        Assert.Contains("Shared access signatures", objectStores);
        Assert.Contains("100 decimal MB", objectStores);
        Assert.Contains("--allow-insecure-source-endpoints", objectStores);
        ...
        Assert.Contains("picket hooks install", hooks);
        Assert.Contains("pre-commit", hooks);
        ...
        Assert.Contains("release-speed", release);
        Assert.Contains("release-minsize", release);
        Assert.Contains("release-diagnostics", release);
        ...
        Assert.Contains("Picket.Security", embedding);
        Assert.Contains("AOT-safe", embedding);
        ...
        Assert.Contains("Picket.Docs", docsGeneratorProject);
        Assert.Contains("CliOptionMetadata.cs", docsGeneratorProject);
        Assert.Contains("Check generated docs", docsWorkflow);
        Assert.Contains("actions/upload-pages-artifact", docsWorkflow);
        Assert.Contains("actions/deploy-pages", docsWorkflow);
        Assert.Contains("pages: write", docsWorkflow);
    ...
    public void GeneratedDocumentationIncludesPublicApiReference()
    {
        ...
        Assert.Contains("items: projectDocsSidebar", siteConfig);
        Assert.Contains("items: apiSidebar", siteConfig);
        Assert.Contains("items: referenceSidebar", siteConfig);
        Assert.DoesNotContain("autogenerate", siteConfig);
        AssertContainsInOrder(siteConfig, s_projectDocsSidebarItems);
        ...
        Assert.Contains("`picket-default` | 45", ruleCatalog);
    ...
    private static void AssertContainsInOrder(string content, string[] values)
    {
        int previousIndex = -1;
        foreach (string value in values)
        {
            Assert.Contains(value, content);
            int currentIndex = content.IndexOf(value, StringComparison.Ordinal);
