...
internal sealed class GcsFixtureServer : IDisposable
{
    ...
    internal GcsFixtureServer(string content)
    {
        _content = content;
    ...
    private async Task HandleAsync(TcpClient client, CancellationToken cancellationToken)
    {
        ...
        if (target.StartsWith("/storage/v1/b/secrets/o?maxResults=1000", StringComparison.Ordinal))
        {
            string json = string.Concat(
                "{\"items\":[{\"name\":\"prod/appsettings.txt\",\"size\":\"",
                Encoding.UTF8.GetByteCount(_content).ToString(CultureInfo.InvariantCulture),
                "\"},{\"name\":\"prod/readme.md\",\"size\":\"2\"}]}");
            await WriteResponseAsync(stream, "application/json", Encoding.UTF8.GetBytes(json), cancellationToken).ConfigureAwait(false);
            return;
        }

        if (target.StartsWith("/storage/v1/b/secrets/o/prod%2Fappsettings.txt?alt=media", StringComparison.Ordinal))
        {
        ...
        if (target.StartsWith("/storage/v1/b/secrets/o/prod%2Freadme.md?alt=media", StringComparison.Ordinal))
        {
