...
internal sealed class AzureBlobFixtureServer : IDisposable
{
    ...
    internal AzureBlobFixtureServer(string content)
    {
        _content = content;
    ...
    internal string LastStorageVersion { get; private set; } = string.Empty;
    ...
    private async Task HandleAsync(TcpClient client, CancellationToken cancellationToken)
    {
        ...
        LastStorageVersion = GetHeaderValue(request, "x-ms-version");
        lock (_requestTargetsLock)
        {
            _requestTargets.Add(target);
        }

        if (target.StartsWith("/secrets?restype=container&comp=list", StringComparison.Ordinal))
        {
            string xml = string.Concat(
                "<?xml version=\"1.0\" encoding=\"utf-8\"?>",
                "<EnumerationResults>",
                "<Blobs>",
                "<Blob><Name>prod/appsettings.txt</Name><Properties><Content-Length>",
                Encoding.UTF8.GetByteCount(_content).ToString(),
                "</Content-Length></Properties></Blob>",
                "<Blob><Name>prod/readme.md</Name><Properties><Content-Length>2</Content-Length></Properties></Blob>",
                "</Blobs>",
                "<NextMarker />",
                "</EnumerationResults>");
